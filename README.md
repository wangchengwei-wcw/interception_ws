...
- **启动坐标系转换**:
  启动坐标系转换节点，由于fastlio定位是定位在无人机自己的局部坐标系下于是需要加偏置进行转换。
  ```bash
  roslaunch rl_policy position_cmd_frame_bridge.launch
  ```
  ```bash
  roslaunch odom_bridge odom_bridge.launch
  ```
...
- **odom_bridge**: 里程计坐标系转换包
  - 包含里程计信息转换节点
  - 该包能够将无人机自己局部坐标系下的坐标通过添加偏置转换到world系下
  - 需要注意无人机初始位置的摆放与偏置的参数设置

- **policy_test**: 仿真测试包
  - 包含目标生成节点和仿真odom节点
  - 目标生成节点能够生成目标队形并检查无人机的初始状态是否满足条件，若不满足则发布控制指令让其满足
  - 仿真odom能够仿造一个odom用来测试策略已经其他节点的使用，odom有多种控制方式用来验证不同方式的控制
