...
class IPPO_RNN(MultiAgent):
    def __init__(
        ...
        """Independent Proximal Policy Optimization (IPPO) with support for Recurrent Neural Networks (RNN, GRU, LSTM, etc.)
            ...
            for uid in self.possible_agents:
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    values, _, outputs = self.values[uid].act(
                        {"states": self._state_preprocessor[uid](states[uid]), **rnn_kwargs}, role="value"
                    )
        ...
        for uid in self.possible_agents:
            ...
            with torch.no_grad(), torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                ...
                last_values, _, _ = value.act(
                    {"states": self._state_preprocessor[uid](self._current_next_states[uid].float()), **rnn_kwargs}, role="value"
                )
                value.train(True)
        ...
        for epoch in range(self._learning_epochs[agent_0]):
            ...
            for i, (
                ...
                with torch.autocast(device_type=self._device_type, enabled=self._mixed_precision):
                    ...
                    # compute value loss
                    predicted_values, _, _ = value.act({"states": sampled_states, **rnn_value_kwargs}, role="value")
