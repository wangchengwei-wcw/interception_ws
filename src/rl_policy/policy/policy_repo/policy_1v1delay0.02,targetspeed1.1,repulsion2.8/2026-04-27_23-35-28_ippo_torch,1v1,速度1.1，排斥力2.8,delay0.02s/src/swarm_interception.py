...
class SwarmInterceptionCfg(DirectMARLEnvCfg):
    ...
    enemy_max_num: int = 1                   # 敌机最多数量（可变编队时使用）
    enemy_min_num: int = 1                   # 敌机最少数量（可变编队时使用）
    ...
    enemy_pursuer_repulsion_max: float = 2.8       # 排斥力上限（防止近距离爆炸）
    ...
    # 友机出生整体偏置：用于测试策略是否依赖 env origin / 世界原点
    friend_spawn_bias_enable: bool = False

    # 固定世界系偏置，单位 m。
    # 例如 (1.0, 0.5) 表示所有友机出生整体在世界系 +x 方向偏 1m，+y 方向偏 0.5m。
    friend_spawn_bias_xy: tuple[float, float] = (1.0, 0.5)

    # 是否每次 reset 随机偏置。
    # False：使用 friend_spawn_bias_xy。
    # True ：每个 env 每次 reset 在圆内随机一个偏置。
    friend_spawn_bias_random: bool = False

    # 随机偏置最大半径，单位 m。仅 friend_spawn_bias_random=True 时生效。
    friend_spawn_bias_random_radius: float = 4.0

    # 观测相关配置
    obs_k_target: int = 1   # 观测最近的多少个敌机
    obs_k_friends: int = 1  # 观测最近的多少个队友
    obs_k_friend_targetpos: int = 1  # 观测最近友机的 Top-K 目标相对位置
    ...
    action_smoothness_penalty_weight: float = 0.05
    near_target_slow_radius: float = 1.5   # 进入该半径后开始逐步限制追击速度；更远处不惩罚高速
    near_target_stop_radius: float = 0.25  # 进入该半径后要求速度接近 hit_safe_speed
    hit_safe_speed: float = 0.3            # 命中/贴近目标时的期望安全速度 (m/s)
    near_target_speed_penalty_weight: float = 0.30
    hit_speed_penalty_weight: float = 3.0
...
class SwarmInterceptionEnv(DirectMARLEnv):
    ...
    def __init__(self, cfg: SwarmInterceptionCfg, render_mode: str | None = None, **kwargs):
        ...
        self._newly_frozen_friend_hit_speed = torch.zeros(N, self.M, device=dev, dtype=dtype)
        ...
        self._goal_e              = None
        # 局部任务参考原点：观测/critic state 不再固定使用 IsaacLab 的 env origin。
        # reset 时会更新为本轮启用友机的初始质心，同时作为敌机目标点。
        self._mission_origin      = self.terrain.env_origins.to(dev, dtype=dtype).clone()
        self._mission_origin[:, 2] = self.terrain.env_origins[:, 2].to(dev, dtype=dtype) + float(self.cfg.enemy_target_alt)
    ...
    def _local_reference_origins(self) -> torch.Tensor:
        """Return the local mission reference used by observations and centralized state.

        This mirrors the multi-wave version's local-reference observation construction,
        but without depending on a multi_wave_enable switch. The reference is updated
        in reset to the enabled friendly UAV centroid, so the policy sees local
        coordinates instead of absolute coordinates tied to the world/env origin.
        """
        if hasattr(self, "_mission_origin") and self._mission_origin is not None:
            return self._mission_origin.to(device=self.device, dtype=self.fr_pos.dtype)
        origins = self.terrain.env_origins.to(device=self.device, dtype=self.fr_pos.dtype)
        local_ref = origins.clone()
        local_ref[:, 2] = origins[:, 2] + float(self.cfg.enemy_target_alt)
        return local_ref

    @torch.no_grad()
    def _update_mission_origin_from_friend_centroid(self, env_ids: torch.Tensor):
        """Use the enabled friendly UAV centroid as local origin and enemy goal point."""
        env_ids = env_ids.to(device=self.device, dtype=torch.long)
        dtype = self.fr_pos.dtype

        enabled = self.friend_enabled[env_ids]
        # 兜底：如果某个 env 没有 enabled friend，就用所有友机，避免除零。
        any_enabled = enabled.any(dim=1, keepdim=True)
        enabled = torch.where(any_enabled, enabled, torch.ones_like(enabled))
        weights = enabled.to(dtype).unsqueeze(-1)  # [N,M,1]
        denom = weights.sum(dim=1).clamp_min(1.0)
        centroid = (self.fr_pos[env_ids] * weights).sum(dim=1) / denom  # [N,3]

        self._mission_origin[env_ids] = centroid
        if self._goal_e is None:
            self._rebuild_goal_e()
        self._goal_e[env_ids] = centroid

    def _rebuild_goal_e(self):
        # 敌机目标点使用局部任务参考点；reset 后该点会被更新为启用友机质心。
        self._goal_e = self._local_reference_origins().clone()
    ...
    def _spawn_enemy(self, env_ids: torch.Tensor):
        ...
        origins_all = self._local_reference_origins()
    ...
    def _continuous_hit_detection(self,
        ...
        hit_speed = torch.linalg.norm(fr_vel_w_step[env_idx, friend_idx], dim=-1)
        self._newly_frozen_friend_hit_speed[env_idx, friend_idx] = hit_speed
    ...
    def _apply_action(self):
        ...
        # print("drone_vel:", torch.linalg.norm(self.fr_vel_w, dim=1))
        # ===== 直接让命中的无人机定住 =====
    ...
    def _get_rewards(self) -> dict[str, torch.Tensor]:
        ...
        near_target_speed_penalty_weight = float(getattr(self.cfg, "near_target_speed_penalty_weight", 0.0))
        hit_speed_penalty_weight = float(getattr(self.cfg, "hit_speed_penalty_weight", 0.0))
        near_target_slow_radius = float(getattr(self.cfg, "near_target_slow_radius", 1.5))
        near_target_stop_radius = float(getattr(self.cfg, "near_target_stop_radius", 0.25))
        hit_safe_speed = float(getattr(self.cfg, "hit_safe_speed", 0.3))
        ...
        near_target_speed_penalty_each = torch.zeros((N, M), device=dev, dtype=dtype)
        if near_target_speed_penalty_weight != 0.0 and E > 0 and near_target_slow_radius > 0.0:
            valid_targets = self._enemy_exists_mask & (~self.enemy_frozen)  # [N,E]
            rel_fe = self.enemy_pos.unsqueeze(1) - self.fr_pos.unsqueeze(2)  # [N,M,E,3]
            dist_fe = torch.linalg.norm(rel_fe, dim=-1)                     # [N,M,E]
            if self._assignment_probs is not None and self._sorted_enemy_idx is not None:
                # 减速只针对当前 agent 自己选择的目标，而不是任意最近目标。
                K_sorted = self._sorted_enemy_idx.shape[-1]
                P_sorted = self._assignment_probs[:, :, :K_sorted]          # [N,M,K]
                j_sorted = P_sorted.argmax(dim=-1)                          # [N,M]
                j_global = self._sorted_enemy_idx.gather(2, j_sorted.unsqueeze(-1)).squeeze(-1)   # [N,M]
                target_dist = dist_fe.gather(2, j_global.unsqueeze(-1)).squeeze(-1)    # [N,M]
                vis_fe = self._gimbal_enemy_visible_mask()                  # [N,M,E]
                valid_enemy_fe = valid_targets.unsqueeze(1) & vis_fe        # [N,M,E]
                has_target = valid_enemy_fe.gather(
                    2, j_global.unsqueeze(-1)
                ).squeeze(-1)                                               # [N,M]
            else:
                # 没有分配缓存时保留兜底逻辑：使用最近可见/活跃目标。
                vis_fe = self._gimbal_enemy_visible_mask()                  # [N,M,E]
                valid_enemy_fe = valid_targets.unsqueeze(1) & vis_fe        # [N,M,E]

                masked_dist = dist_fe.masked_fill(~valid_enemy_fe, float("inf"))
                target_dist = masked_dist.min(dim=-1).values                # [N,M]
                has_target = torch.isfinite(target_dist)
            slow_radius = max(near_target_slow_radius, near_target_stop_radius + 1e-6)
            stop_radius = max(near_target_stop_radius, 0.0)
            max_pursuit_speed = math.sqrt(float(self.cfg.v_max_xy) ** 2 + float(self.cfg.v_max_z) ** 2)
            max_pursuit_speed = max(max_pursuit_speed, hit_safe_speed)
            ramp = ((target_dist - stop_radius) / (slow_radius - stop_radius)).clamp(0.0, 1.0)
            allowed_speed = hit_safe_speed + ramp * (max_pursuit_speed - hit_safe_speed)
            speed = torch.linalg.norm(self.fr_vel_w, dim=-1)                # [N,M]
            inside_slow = target_dist <= slow_radius
            inside_stop = target_dist <= stop_radius
            overspeed_sq = torch.clamp(speed - allowed_speed, min=0.0).square()

            # 只惩罚超过 hit_safe_speed 的速度；低于阈值不惩罚
            stop_speed_sq = torch.clamp(speed - hit_safe_speed, min=0.0).square()
            near_target_speed_penalty_each = torch.where(
                inside_stop,
                stop_speed_sq,
                overspeed_sq,
            )
            near_target_speed_penalty_each = (
                near_target_speed_penalty_each
                * inside_slow.to(dtype)
                * has_target.to(dtype)
                * friend_active_f
            )

        # ———————————————————— 命中瞬间超速惩罚 ————————————————————
        hit_speed_overshoot = torch.clamp(
            self._newly_frozen_friend_hit_speed - hit_safe_speed,
            min=0.0,
        )
        hit_speed_penalty_each = hit_speed_overshoot.square() * self._newly_frozen_friend.to(dtype)
        ...
        r_each = r_each - near_target_speed_penalty_weight * near_target_speed_penalty_each
        r_each = r_each - hit_speed_penalty_weight * hit_speed_penalty_each
        ...
        self._newly_frozen_friend_hit_speed[:] = 0.0

        # ========================== 日志项 ==========================
        reward_terms = {
            "hit":                   hit_weight * per_agent_hit,
            "near_target_speed":     -near_target_speed_penalty_weight * near_target_speed_penalty_each,
            "hit_speed_penalty":     -hit_speed_penalty_weight * hit_speed_penalty_each,
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        # 本轮 reset 先使用 env origin 作为临时局部参考，避免沿用上一回合的质心。
        # 等友机出生并应用 enabled mask 后，再更新为启用友机初始质心。
        if hasattr(self, "_mission_origin") and self._mission_origin is not None:
            self._mission_origin[env_ids] = origins.to(device=dev, dtype=dtype)
            self._mission_origin[env_ids, 2] = origins[:, 2].to(device=dev, dtype=dtype) + float(self.cfg.enemy_target_alt)
            if self._goal_e is not None:
                self._goal_e[env_ids] = self._mission_origin[env_ids]
        ...
        self._newly_frozen_friend_hit_speed[env_ids] = 0.0
        ...
        # ---------------------------------------------------------------------
        # 目的：
        #   让友机不要总是在 env origin / 原点出生，用来测试策略是否依赖绝对坐标。
        #
        # 逻辑：
        #   - friend_spawn_bias_enable=False：
        #       不加偏置，保持原逻辑。
        #   - friend_spawn_bias_enable=True 且 friend_spawn_bias_random=False：
        #       使用固定世界系偏置 friend_spawn_bias_xy。
        #   - friend_spawn_bias_enable=True 且 friend_spawn_bias_random=True：
        #       每个 env 每次 reset 在半径 friend_spawn_bias_random_radius 的圆内随机偏置。
        #
        # 注意：
        #   这里只平移友机出生点，不改变队形内部 offsets_xy。
        #   后面 self._update_mission_origin_from_friend_centroid(env_ids)
        #   会把偏置后的启用友机质心写入 _mission_origin 和 _goal_e。
        # =====================================================================
        spawn_bias_xy = torch.zeros(N, 2, device=dev, dtype=dtype)

        if getattr(self.cfg, "friend_spawn_bias_enable", False):
            if getattr(self.cfg, "friend_spawn_bias_random", False):
                bias_radius_max = float(getattr(self.cfg, "friend_spawn_bias_random_radius", 1.0))

                # 圆内均匀随机：r = R * sqrt(u)，theta = 2pi * v
                u = torch.rand(N, device=dev, dtype=dtype)
                v = torch.rand(N, device=dev, dtype=dtype)
                r_bias = bias_radius_max * torch.sqrt(u.clamp_min(1e-12))
                theta_bias = 2.0 * math.pi * v

                spawn_bias_xy[:, 0] = r_bias * torch.cos(theta_bias)
                spawn_bias_xy[:, 1] = r_bias * torch.sin(theta_bias)
            else:
                bx, by = getattr(self.cfg, "friend_spawn_bias_xy", (1.0, 0.5))
                spawn_bias_xy[:, 0] = float(bx)
                spawn_bias_xy[:, 1] = float(by)

        # 4.2) 转换到世界坐标系 [N, M, 3]
        fr0 = torch.empty(N, self.M, 3, device=dev, dtype=self.fr_pos.dtype)

        # 原来是：
        #   origins[:, :2].unsqueeze(1) + offsets_xy
        # 现在变成：
        #   origins[:, :2].unsqueeze(1) + spawn_bias_xy.unsqueeze(1) + offsets_xy
        fr0[..., :2] = origins[:, :2].unsqueeze(1) + spawn_bias_xy.unsqueeze(1) + offsets_xy
        ...
        # 使用本轮启用友机的初始质心作为：
        # 1) 观测/critic state 的局部参考原点；2) 敌机的目标点。
        self._update_mission_origin_from_friend_centroid(env_ids)
        self._refresh_enemy_cache()

        # 重算初速度，保证 reset 后第一个观测里的 enemy_vel 与新的目标点/运动模式一致
    ...
    def _get_observations(self) -> dict[str, torch.Tensor]:
        ...
        # 输出维度：3 位置 + 3 速度
        # TODO：考虑是否需要加入姿态信息
        env_origins_expanded = self._local_reference_origins().unsqueeze(1)  # [N, 1, 3]
    ...
    def _get_states(self) -> torch.Tensor:
        ...
        # ===== 1. 友机状态：位置、速度、姿态 =====
        env_origins_expanded = self._local_reference_origins().unsqueeze(1)  # [N, 1, 3]
        friend_pos_local = self.fr_pos - env_origins_expanded               # [N, M, 3] - 相对局部任务原点位置
        ...
        # ===== 2. 敌机状态：位置、速度 =====
        enemy_pos_local = self.enemy_pos - env_origins_expanded  # [N, E, 3] - 相对局部任务原点位置
        ...
        # ===== 3. 敌机团中心位置 =====
        enemy_centroid_relative = self._enemy_centroid - env_origins_expanded.squeeze(1)  # [N, 3] - 相对局部任务原点
